...
#ifdef _CRT_INSECURE_DEPRECATE
    sprintf_s(szLogFile, ARRAYSIZE(szLogFile), "%s.%08d.xml", s_szLogFile, nClient);
#else
    sprintf(szLogFile, "%s.%08d.xml", s_szLogFile, nClient);
#endif
...
DWORD main(int argc, char **argv)
{
...
#ifdef _CRT_INSECURE_DEPRECATE
    sprintf_s(s_szPipe, ARRAYSIZE(s_szPipe), "%s.%d", TBLOG_PIPE_NAME, GetCurrentProcessId());
#else
    ...
    for (; arg < argc && (argv[arg][0] == '-' || argv[arg][0] == '/'); arg++) {
        ...
        switch (argn[0]) {
          ...
          case 'O':
#ifdef _CRT_INSECURE_DEPRECATE
            strcpy_s(s_szLogFile, ARRAYSIZE(s_szLogFile), argp);
#else
...
#ifdef _CRT_INSECURE_DEPRECATE
    strcpy_s(pszFilePart, szExePath + ARRAYSIZE(szExePath) - pszFilePart,
             "trcbld" DETOURS_STRINGIFY(DETOURS_BITS) ".dll");
    strcpy_s(szDllPath, ARRAYSIZE(szDllPath), szExePath);
#else
    strcpy(pszFilePart,
           "trcbld" DETOURS_STRINGIFY(DETOURS_BITS) ".dll");
    ...
    wcscpy_s(s_Payload.wzStdin, ARRAYSIZE(s_Payload.wzStdin), L"\\\\.\\CONIN$");
    wcscpy_s(s_Payload.wzStdout, ARRAYSIZE(s_Payload.wzStdout), L"\\\\.\\CONOUT$");
    wcscpy_s(s_Payload.wzStderr, ARRAYSIZE(s_Payload.wzStderr), L"\\\\.\\CONOUT$");
    wcscpy_s(s_Payload.wzParents, ARRAYSIZE(s_Payload.wzParents), L"");
