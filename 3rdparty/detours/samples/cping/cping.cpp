...
STDAPI PingMessage(PCSTR msg, ...)
{
    ...
    vsprintf_s(s_szMessageBuf, ARRAYSIZE(s_szMessageBuf), msg, args);
#else
...
#ifdef _CRT_INSECURE_DEPRECATE
        vsprintf_s(s_szMessageBuf, ARRAYSIZE(s_szMessageBuf), pszMsg, args);
#else
...
#ifdef _CRT_INSECURE_DEPRECATE
    wcscpy_s(wzKey, ARRAYSIZE(wzKey), pwzKey);
#else
    wcscpy(wzKey, pwzKey);
#endif
                              ...
                              PWCHAR pwzValueName, PWCHAR pwzValue)
{
    ...
    if (pwzSubkey != NULL) {
#ifdef _CRT_INSECURE_DEPRECATE
        wcscat_s(wzKey, ARRAYSIZE(wzKey), L"\\");
        wcscat_s(wzKey, ARRAYSIZE(wzKey), pwzSubkey);
#else
        wcscat(wzKey, L"\\");
...
static void Register(void)
{
...
#ifdef _CRT_INSECURE_DEPRECATE
        wcscpy_s(wzName, ARRAYSIZE(wzName), pwz + 1);
#else
        wcscpy(wzName, pwz + 1);
#endif
    }
...
#ifdef _CRT_INSECURE_DEPRECATE
        wcscpy_s(wzName, ARRAYSIZE(wzName), wzModule);
#else
    ...
    // printf("  Class: %ls\n", wzClass);
#ifdef _CRT_INSECURE_DEPRECATE
    wcscpy_s(wzKey, ARRAYSIZE(wzKey), L"CLSID\\");
    wcscat_s(wzKey, ARRAYSIZE(wzKey), wzClass);
#else
...
#ifdef _CRT_INSECURE_DEPRECATE
    wcscpy_s(wzKey, ARRAYSIZE(wzKey), L"AppID\\");
    wcscat_s(wzKey, ARRAYSIZE(wzKey), wzClass);
#else
...
#ifdef _CRT_INSECURE_DEPRECATE
    wcscpy_s(wzKey, ARRAYSIZE(wzKey), L"AppID\\");
    wcscat_s(wzKey, ARRAYSIZE(wzKey), wzName);
#else
    wcscpy(wzKey, L"AppID\\");
    wcscat(wzKey, wzName);
#endif
...
#ifdef _CRT_INSECURE_DEPRECATE
    wcscpy_s(wzKey, ARRAYSIZE(wzKey), L"Software\\Microsoft\\Detours\\ProcessorCycles");
#else
...
#ifdef _CRT_INSECURE_DEPRECATE
        swprintf_s(wzValue, ARRAYSIZE(wzValue), L"%d", cycles);
#else
               ...
               BOOLEAN fToClient, LONG cbPacket, LONG nCount)
{
    ...
    if (hNetwork != NULL) {
        ...
        if (fToClient) {
#ifdef _CRT_INSECURE_DEPRECATE
            swprintf_s(wzKey, ARRAYSIZE(wzKey), L"ToClient\\%d", cbPacket);
#else
...
#ifdef _CRT_INSECURE_DEPRECATE
            swprintf_s(wzKey, ARRAYSIZE(wzKey), L"ToServer\\%d", cbPacket);
#else
            swprintf(wzKey, L"ToServer\\%d", cbPacket);
#endif
        }
#ifdef _CRT_INSECURE_DEPRECATE
        swprintf_s(wzLatency, ARRAYSIZE(wzLatency), L"%I64d", msAvg);
#else
    ...
    for (; arg < argc; arg++) {
        if (argv[arg][0] == '-' || argv[arg][0] == '/') {
            ...
            switch (argn[0]) {
              ...
              case '\0':                                // Local Host
#ifdef _CRT_INSECURE_DEPRECATE
                wcscpy_s(wzServers[nServers++], ARRAYSIZE(wzServers[nServers++]), L"localhost");
#else
...
#ifdef _CRT_INSECURE_DEPRECATE
            wcscpy_s(wzServers[nServers++], ARRAYSIZE(wzServers[nServers++]), argv[arg]);
#else
...
#ifdef _CRT_INSECURE_DEPRECATE
                    wcscpy_s(g_wzServerName, ARRAYSIZE(g_wzServerName), wzServers[n] + 2);
#else
...
#ifdef _CRT_INSECURE_DEPRECATE
                    wcscpy_s(g_wzServerName, ARRAYSIZE(g_wzServerName), wzServers[n]);
#else
