...
void DumpModuleInfo(HMODULE hModule)
{
    ...
    if (pdir[IMAGE_DIRECTORY_ENTRY_COM_DESCRIPTOR].VirtualAddress != 0 &&
        ...
        if ((pch->Flags & 0x3) == 0x0) {
            sprintf(szClr, "clr   ");   // 32- or 64-bit.
        }
        else if ((pch->Flags & 0x3) == 0x1) {
            sprintf(szClr, "clri  ");   // IL-Only, 32- or 64-bit.
        }
        else if ((pch->Flags & 0x3) == 0x2) {
            sprintf(szClr, "clr32 ");   // must be 32-bit.
        }
        else if ((pch->Flags & 0x3) == 0x3) {
            sprintf(szClr, "clr32i");   // IL-Only, must be 32-bit.
        }
    }
    else {
        sprintf(szClr, "      ");
    ...
    if (pinh->FileHeader.Machine == 0x8664) {
        sprintf(szMachine, "x64", pinh->FileHeader.Machine);
    }
    else if (pinh->FileHeader.Machine == 0x014c) {
        sprintf(szMachine, "x86", pinh->FileHeader.Machine);
    }
    else if (pinh->FileHeader.Machine == 0x0200) {
        sprintf(szMachine, "i64", pinh->FileHeader.Machine);
    }
    else if (pinh->FileHeader.Machine == 0x01c0) {
        sprintf(szMachine, "arm", pinh->FileHeader.Machine);
    }
    else {
        sprintf(szMachine, "%04x", pinh->FileHeader.Machine);
        DWORD dwSize = DetourGetSizeOfPayloads(hModule);
        if (dwSize > 0) {
            sprintf(szMachine, "     ");
            sprintf(szFile, "-- %d byte payload.", dwSize);
        }
    }
