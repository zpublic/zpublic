...
int __cdecl main(int argc, char **argv)
{
    ...
    if (!SearchPath(NULL, pszExe, ".exe", ARRAYSIZE(szFullExe), szFullExe, &pszFileExe)) {
        pszExe = "size" DETOURS_STRINGIFY(DETOURS_BITS) ".exe";
        SearchPath(NULL, pszExe, ".exe", ARRAYSIZE(szFullExe), szFullExe, &pszFileExe);
    }

    sprintf_s(szCommand, sizeof(szCommand), "%s %d", pszExe, repeats - 1);
    ...
    SetLastError(0);
    if (!CreateProcess(szFullExe[0] ? szFullExe : NULL, szCommand,
