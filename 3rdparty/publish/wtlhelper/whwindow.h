...
class IWHHtmlContainerWindowCallback
{
public:
	virtual void OnNavigateComplete(int id) = 0;
	virtual void OnNavigateError(int id) = 0;
};
...
public:
	...
	void SetCallback( IWHHtmlContainerWindowCallback* opCB )
	{
		m_linkerCB = opCB;
	}

    HWND Create(HWND hWndParent, UINT nID, BOOL bVisible = TRUE, COLORREF crBg = RGB(0xFF, 0xFF, 0xFF), bool bMouseWheel = true)
    {
...
//         ::CloseHandle(::CreateThread(NULL, 0, _CreateCtrlProc, this, 0, NULL));
	...
	void SetDefaultUrl(LPCTSTR lpszDefaultUrl)
	{
		if (lpszDefaultUrl)
			m_strDefaultUrl = lpszDefaultUrl;
	}
protected:

	IWHHtmlContainerWindowCallback			*m_linkerCB;
    CAxWindow m_wndIE;
    CString m_strUrl;
    ...
    void WINAPI NavigateComplete2WebAdinfo(LPDISPATCH pDisp, VARIANT* URL)
    {
        if (!m_bNavigateError)
        {
			...
			if (m_linkerCB)
				m_linkerCB->OnNavigateComplete(GetDlgCtrlID());
        }
    }
        ...
        )
    {
		...
		if (pvtURL != NULL && m_linkerCB)
			m_linkerCB->OnNavigateError(GetDlgCtrlID());
...
//         }
    ...
    void OnTimer(UINT_PTR nIDEvent)
    {
        ...
        if (piWebBrowser)
        {
            piWebBrowser->put_RegisterAsDropTarget( VARIANT_FALSE );
            piWebBrowser->put_Silent( VARIANT_TRUE );
            piWebBrowser->Navigate(CComBSTR(m_strUrl), &flag, &v, &v, &v);
        }
    ...
    void _CreateIEControl(HWND hWnd)
    {
		...
		if (!m_strDefaultUrl.IsEmpty())
		{
			...
			{
				piWebBrowser->put_RegisterAsDropTarget( VARIANT_FALSE );
				piWebBrowser->Navigate(CComBSTR(m_strDefaultUrl), &v, &v, &v, &v);
			}
		}
    }
