...
// 附注     : 
// -------------------------------------------------------------------------
const BYTE* KMD5::GetMD5(const TCHAR *pszFileName, ULONG uSize, PFN_Filter pFilter)
{
    ...
    if (uSize == 0)
    {
        if (!m_pbyBuf)
        {
            ...
            ::RtlZeroMemory(m_pbyBuf, NMAX_BUF);
        }

        //size_t nRead = 0;
        //while (0 != (nRead = ::fread(m_pbyBuf, sizeof(BYTE), NMAX_BUF, pFile)))
        //{
        //    m_pAlgoMD5->MD5Update(m_pbyBuf, nRead);
        //}

        while (::ReadFile(hFile, m_pbyBuf, NMAX_BUF, &ulNumberOfBytesRead, NULL) && ulNumberOfBytesRead)
        {
            m_pAlgoMD5->MD5Update(m_pbyBuf, ulNumberOfBytesRead);
        }
    }
    else
    {
        ...
        m_pbyBuf = new BYTE[uSize + 1];
        ::RtlZeroMemory(m_pbyBuf, uSize + 1);

        //ULONG nRead = 0;
        //if (0 != (nRead = (ULONG)::fread(m_pbyBuf, sizeof(BYTE), uSize, pFile)))
        //{
        //    if (pFilter != NULL)
        //    {
        //        pFilter(m_pbyBuf, nRead);
        //    }

        //    m_pAlgoMD5->MD5Update(m_pbyBuf, nRead);
        //}

        if (::ReadFile(hFile, m_pbyBuf, NMAX_BUF, &ulNumberOfBytesRead, NULL) && ulNumberOfBytesRead)
        {
            if (pFilter)
            {
                pFilter(m_pbyBuf, ulNumberOfBytesRead);
            }
...
const TCHAR* KMD5::GetMD5StrEx(const TCHAR *pszFileName, ULONG uSize, PFN_Filter pFilter)
{
    if (!GetMD5(pszFileName, uSize, pFilter))
    {
        return NULL;
    }
