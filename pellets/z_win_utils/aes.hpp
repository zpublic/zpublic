 ...
 * @brief AES算法实现
 *
 *   ZLRijndael : AES中使用到的基本密码算法
 *   ZLAes      : AES算法
 *
 * @todo 坑:PADDING_PKCS7, CFB, OFB尚未实现
 * @attention 感觉有BUG,暂时请不要使用
 */

#pragma once
#include <string.h>

#define ZLRIJNDAEL_KEYLENGTH(keybits) ((keybits)/8)
#define ZLRIJNDAEL_RKLENGTH(keybits)  ((keybits)/8+28)
#define ZLRIJNDAEL_NROUNDS(keybits)   ((keybits)/32+6)
#define ZLRIJNDAEL_FULL_UNROLL
#define ZLRIJNDAEL_AES_MAXNR 14
#define ZLRIJNDAEL_RK_SIZE   (4 *(ZLRIJNDAEL_AES_MAXNR + 1))
...
/**
 * @brief Rijndael算法, 是AES中使用到的基本密码算法
 *
 *   摘至一个自称由连续四个字母组成的奇怪网站: http://www.efgh.com
 *   有删改
 */
class ZLRijndael
{
...
public:
    ...
    static void Encrypt(const unsigned long *rk, int nrounds, const unsigned char plaintext[16], unsigned char ciphertext[16]);
    static void Decrypt(const unsigned long *rk, int nrounds, const unsigned char ciphertext[16], unsigned char plaintext[16]);
};
 ...
 * @note: 注:待加密内容和输出内容,可以是同一块内存区.方便使用.但请预留足够大小存放输出内容.
 * @todo 坑:PADDING_PKCS7, CFB, OFB尚未实现
 * @attention 感觉有BUG,暂时请不要使用
 */
class ZLAes
...
__declspec(selectany) const ZLRijndael::u32 ZLRijndael::Te0[] = 
{
...
__declspec(selectany) const ZLRijndael::u32 ZLRijndael::Te1[] = 
{
...
__declspec(selectany) const ZLRijndael::u32 ZLRijndael::Te2[] = 
{
...
__declspec(selectany) const ZLRijndael::u32 ZLRijndael::Te3[] = 
{
...
__declspec(selectany) const ZLRijndael::u32 ZLRijndael::Te4[] = 
{
...
__declspec(selectany) const ZLRijndael::u32 ZLRijndael::Td0[] = 
{
...
__declspec(selectany) const ZLRijndael::u32 ZLRijndael::Td1[] = 
{
...
__declspec(selectany) const ZLRijndael::u32 ZLRijndael::Td2[] = 
{
...
__declspec(selectany) const ZLRijndael::u32 ZLRijndael::Td3[] = 
{
...
__declspec(selectany) const ZLRijndael::u32 ZLRijndael::Td4[] = 
{
...
__declspec(selectany) const ZLRijndael::u32 ZLRijndael::rcon[] = 
{
 ...
 */
inline int ZLRijndael::SetupEncrypt( unsigned long *rk, const unsigned char *key, int keybits )
{
  int i = 0;
  u32 temp;
 ...
 * @return the number of rounds for the given cipher key size.
 */
inline int ZLRijndael::SetupDecrypt( unsigned long *rk, const unsigned char *key, int keybits )
{
  int nrounds, i, j;
  u32 temp;

  /* expand the cipher key: */
  nrounds = SetupEncrypt(rk, key, keybits);
...
inline void ZLRijndael::Encrypt( const unsigned long *rk, int nrounds, const unsigned char plaintext[16], unsigned char ciphertext[16] )
{
...
inline void ZLRijndael::Decrypt( const unsigned long *rk, int nrounds, const unsigned char ciphertext[16], unsigned char plaintext[16] )
{
  u32 s0, s1, s2, s3, t0, t1, t2, t3;
  #ifndef FULL_UNROLL
    int r;
